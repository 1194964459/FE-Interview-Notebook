...
```
* TODO:**若 “状态值不变” 时**，两者触发渲染的 逻辑是不一样的：
  * setState会重复触发，
  * useState只触发一次 后面值相同的话 不会再触发了   

  原因：**useState 内部做了一层浅比较优化**：如果新状态与旧状态相同（```Object.is(newValue, oldValue)```），则不会触发重新渲染，以此提升性能。


## 注意事项
状态更新是异步批量的，依赖前状态用「函数形式」；
状态是不可变的，修改引用类型必须返回新值；
状态更新只能在「非渲染阶段」执行（事件、副作用、异步回调）；

避免在渲染阶段执行“setState”、“useState”修改状态。

类组件避免在 shouldComponentUpdate 中触发 setState

禁止：组件顶层（直接写在类体/函数体中）用 if/for 调用 setState（修改状态）；

```js
class BadClassComponen extends  React.Component {

  // TODO:禁止：组件顶层代码（每次渲染都会执行）
  for (let i = 0; i < 3; i++) {
    // 每次调用都基于旧状态 this.state.count（初始 0），最终只更新为 1
    this.setState({ count: this.state.count + 1 });
  }

  // 这种是允许的
 handleClick = () => {
    // 事件回调中执行 for 循环 + setState，不会触发无限渲染
    for (let i = 0; i < 3; i++) {
      // 用函数形式获取最新状态，最终 count 会从 0 变成 3
