...
**（4）返回值是 Promise**
**async函数的返回值是 Promise 对象**，这比 Generator 函数的返回值是 *Iterator 对象*方便多了。
...
如果await后面的异步操作出错，那么等同于async函数返回的 Promise 对象被reject。如果不想因为这个而影响async函数返回promise对象状态的改变（rejected），可以用try...catch而捕获一下。
