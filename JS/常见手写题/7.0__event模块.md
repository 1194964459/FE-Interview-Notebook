...
EventEmitter.prototype.addListener = function(type, fn, once = false) {
    ...
    if (!hanlder) {
        this.events.set(type, wrapCallback(fn, once));   // 没有type绑定事件
    } else if (hanlder && typeof hanlder.callback === 'function') {
        this.events.set(type, [hanlder, wrapCallback(fn, once)]);  // 目前type事件只有一个回调
    } else {
        hanlder.push(wrapCallback(fn, once));  // 目前type事件数>=2
    }
}
