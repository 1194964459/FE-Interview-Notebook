...
**关键实现步骤：**把原型作为另一个类型的实例。原理见：[原型及原型链]()
...
**3. 原型链的问题**
* 原型中包含的引用值会在所有实例间共享。这也是为什么属性通常会在构造函数中定义而不会定义在原型上的原因。在使用原型实现继承时，原型实际上变成了另一个类型的实例。这意味着原先的实例属性摇身一变成为了原型属性。具体原因在[创建对象一文 中'原型的问题']()也提到过。
