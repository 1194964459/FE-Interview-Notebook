...
**2. URL 解析**
> 解析之前转义过的url，因为url如果不转义会出现歧义。比如`http:www.baidu.com?key=value`，假如我的key本身就包括等于=符号，比如ke=y=value，就会出现歧义。（参考：[URL参数解析 及 解析一个完整的url](../常见手写题/5.2__URL%20Params解析.md)）

**3. DNS解析，将域名转化为 IP地址**（见：[dns解析流程](https://github.com/1194964459/FE-Interview-Notebook/blob/main/Network/1.0.1__dns%E5%8F%8Adns%E8%A7%A3%E6%9E%90.md)）

**4. 与服务器建立连接** （见：[三次握手 与 四次挥手](./1.1__TCP.md)）
...
## 二、URL解析详情补充
参考：[URL 完整结构](./location_referer_origin比较.md)

**1. `encodeURIComponent 与 encodeURI `区别及联系**：

以`http://www.jxbh.cn/illegal value.htm`为例：

* encodeURI()主要用于整个URI（`http://www.jxbh.cn/illegal value.htm`）
* encode-URIComponent()主要用于对URI中的某一段进行编码（如前面URI中的`illegal value.htm`），转义的字符比较多
...
这些自上而下渲染解析代码有些是同步的，但是有些操作也是异步的，比如：
 1. 遇到CSS资源的加载。
    ...
    * 遇到的是```<link></link>```外联样式。<br />
      > => **异步**地进行处理，开辟一个新的http网络请求线程。(注意：同一个源下，根据不同的浏览器，最多只允许同时开辟4~7个HTTP线程，也就是“HTTP的并发数”)。<br />
...
1. 遇到```<script></script>```资源的请求<br />
   ...
   defer与async：脚本都是异步下载的，不阻塞 HTML 解析，区别是：   
    defer：
      * 脚本异步下载（不阻塞 HTML 解析），脚本会在整个**HTML解析完成后、DOMContentLoaded 事件触发前**执行。
      * 多个带 defer的脚本会按照在 HTML 中出现的顺序执行（先出现的先执行）。
      * 适合**有依赖关系、需要访问 DOM** 的脚本，保证顺序和执行时机。
    
    async：
      * 脚本下载完成后立即执行（此时可能 HTML 尚未解析完成）
      * 多个带 async 的脚本执行顺序不确定，完全取决于下载速度（谁先下载完谁先执行）。
      * 适合**独立、无依赖、无需 DOM** 的脚本

2. 遇到```<img />、<audio />```等音视频资源。<br />
   > => 默认都是异步的，也会发送HTTP网络线程去请求加载对应的资源文件，不阻碍GUI的渲染，当GUI渲染完成之后，才会将请求回来的资源进行解析。

## 四、性能优化及指标
[前端性能常见优化手段](../前端性能优化/常见优化手段.md)
...
FP：First Paint  // 第一个像素点绘制
FCP：First Contentful Paint   // 第一个内容绘制

LCP：Largest Contentful Paint  // 视口内最大的内容完成绘制的时间，< 2.5秒
FID：First Input Delay  // 首次输入延迟，用户首次与页面交互（click）到浏览器开始处理该交互的时间差，≤ 100 毫秒
CLS：Cumulative Layout Shift  // 累积布局偏移，解决：动态占位、为图片/视频等设置宽高比
```
